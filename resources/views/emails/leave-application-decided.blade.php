...
<body>
    ...
    <p>{{ $decide == 'denied' ? 'We regret to inform you that your' : 'Your' }} leave application has been
        {{ $decide }} by your supervisor. Here are the details of the application:</p>
    <ul>
        {{-- <li><em>Name:</em>&nbsp;<strong>{{ $dLeave->name }}</strong></li> --}}
        <li><em>Leave Number:</em>&nbsp;<strong>{{ $dLeave['control_number'] }}</strong></li>
        <li><em>Leave
                Type:</em>&nbsp;<strong>{{ $dLeave['leave_type'] != 'Others' ? $dLeave['leave_type'] : $dLeave['leave_type'] . ' - ' . $dLeave['others'] }}</strong>
        </li>
        <li><em>From:</em>&nbsp;<strong>{{ \Carbon\Carbon::parse($dLeave['date_from'])->format('D, m/d/Y') }}</strong>
        </li>
    ...
    @if ($decide == 'denied')
        <ul>
            <li><em>Reason for Denial:</em>&nbsp;<p><strong>{{ $dLeave['action_reason'] }}</strong></p>
            </li>
        </ul>
    @endif
