    ...
    <div class="max-w-7xl mx-auto py-2 sm:px-6 lg:px-8">
        ...
        <form id="update-leave-form" method="POST" action="{{ route('hris.leave.view-leave-details') }}">
            ...
            for ($n = 0; $n < count($notif); $n++) {
                if ($notif[$n] == "IN PERSON") {
                    $inperson = "checked";
                }
                if ($notif[$n] == "BY SMS") {
                    $bysms = "checked";
                }
                if ($notif[$n] == "BY E-MAIL") {
                    $byemail = "checked";
                }
            }
            ?>
            ...
            <div class="px-4 py-5 bg-white sm:p-6 shadow {{ isset($actions) ? 'sm:rounded-tl-md sm:rounded-tr-md' : 'sm:rounded-md' }}">
                <div class="grid grid-cols-5 gap-6">
                    ...
                    <div class="col-span-5 sm:col-span-1">
                        <x-jet-label for="reason" value="{{ __('REASON') }}" class="font-semibold text-gray-800 leading-tight" />
                        <textarea id="reason" name="reason" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full" wire:model.defer="state.reason"> </textarea>
            ...
            <div class="px-4 py-5 bg-white sm:p-6 shadow {{ isset($actions) ? 'sm:rounded-tl-md sm:rounded-tr-md' : 'sm:rounded-md' }}">
                <div class="grid grid-cols-5 gap-6">
                    ...
                    <div class="col-span-3 sm:col-span-1">
                        ...
                        <div class="col-span-5 sm:col-span-1">
                            <x-jet-label for="reason" value="{{ __('REASON') }}" class="font-semibold text-gray-800 leading-tight" />
                            <textarea id="reason" name="reason" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full" wire:model.defer="state.reason"> {{ $leave->reason }} </textarea>
                            <x-jet-input-error for="reason" class="mt-2" />
