...
<x-guest-layout>
    ...
    <div class="max-w-6xl mx-auto mt-4">
        ...
        <div
            class="px-5 pt-3 bg-white sm:p-6 shadow {{ isset($actions) ? 'sm:rounded-tl-md sm:rounded-tr-md' : 'sm:rounded-md' }}">
            @if ($dLeave == '')
                ...
                <div class="row border-1 px-2">
                    ...
                    <div class="col-md-3 px-1 mt-1">
                        <x-jet-label class="text-secondary" for="employee_number">
                            {!! __('<i class="text-sm">Employee ID:</i>&nbsp;<strong>:employee_number</strong>', [
                                'employee_number' => $dLeave->employee_id,
                            ]) !!}
                        </x-jet-label>
                    </div>
                    <div class="col-md-3 p-1 text-wrap">
                        <x-jet-label class="text-secondary" for="office">
                            {!! __('<i class="text-sm">Office:</i>&nbsp;<strong>:office</strong>', [
                                'office' => strtoupper($dLeave->office),
                            ]) !!}
                        </x-jet-label>
                    </div>
                    <div class="col-md-3 p-1 text-wrap">
                        <x-jet-label class="text-secondary" for="department">
                            {!! __('<i class="text-sm">Department:</i>&nbsp;<strong>:department</strong>', [
                                'department' => strtoupper($dLeave->department),
                            ]) !!}
                        </x-jet-label>
                    </div>
                </div>
                ...
                <div class="row border-1 px-2">
                            ...
                            @if ($dLeave->leave_status == 'Pending')
                                <div class="form-floating">
                                    <select name="dLtype" id="dLtype"
                                        class="form-control border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block"
                                        placeholder="LEAVE TYPE">
                                        @foreach ($leaveTypes as $leave_type)
                                            <option value="{{ $leave_type->leave_type }}"
                                                @if ($dLeave->leave_type == $leave_type->leave_type) selected @endif>
                                ...
                                <div id="divLOthers" name="divLOthers" @if ($dLeave->leave_type != 'Others') hidden @endif>
                                    <x-jet-input id="dLOthers" name="dLOthers" class="w-full" type="text"
                                        placeholder="Specify leave here..." value="{{ $dLeave->others }}"
                                        autocomplete="off" />
                                </div>
                            @else
                                <x-jet-label class="text-secondary text-wrap" for="leaveType">
                                    {!! __('<i class="text-sm">Leave Type:</i>&nbsp;<strong>:leave_type</strong>', [
                                        'leave_type' => strtoupper($dLeave->leave_type_name),
                                    ]) !!}
                                </x-jet-label>
                    ...
                    <div class="col-md-4 p-1">
                        <x-jet-label id="dLDate" class="text-secondary text-wrap" for="no_of_days">
                            {!! __('<i class="text-sm">Date:</i>&nbsp;<strong>:date_covered</strong>', [
                                'date_covered' =>
                                    '(' .
                                    join('<b class="px-1">-</b>', [
                    ...
                    <div class="col-md-2 p-1">
                        <x-jet-label class="text-secondary" for="date_applied">
                            {!! __('<i class="text-sm">Date Applied:</i><br><strong>:date_applied</strong>', [
                                'date_applied' => date('m/d/Y g:i A', strtotime($dLeave->date_applied)),
                            ]) !!}
                        </x-jet-label>
                    </div>
                </div>
                                ...
                                <table class="table table-bordered data-table mx-auto text-sm">
                                    ...
                                    <tbody id="viewLeaveCredits">
                                        <tr class="text-center bg-faded">
                                            ...
                                            <td>{{ $dLeave->gender == 'M' ? $leaveCredits->PL : $leaveCredits->ML }}
                                            </td>
                        ...
                        <div class="col-span-5 sm:col-span-1 sm:justify-start">
                            <table class="table table-bordered mx-auto text-sm">
                                <tr>
                                    <td><em>Status</em></td>
                                    <th
                                        class="{{ $dLeave->leave_status != 'Pending' ? 'font-weight-bold ' . ($dLeave->leave_status == 'Cancelled' || $dLeave->leave_status == 'Denied' ? 'red-color' : 'green-color') : '' }}">
                                        {{ $dLeave->leave_status }}
                                    </th>
                                </tr>
                                <tr>
...
<script type="text/javascript">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelector('#leaveAction').addEventListener('click', function() {
            ...
            const dLDate =
                `{{ join(' - ', [date('D, m/d/Y', strtotime($dLeave->date_from)), date('D, m/d/Y', strtotime($dLeave->date_to))]) }}`;
