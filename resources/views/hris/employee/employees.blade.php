                ...
                <div class="col-span-8 sm:col-span-8 sm:justify-center">
                    <div id="filter_fields" class="form-group border-0 col-md-12 py-1 gap-2 inset-shadow">
                        <div class="row pb-1">
                            <div class="col-sm-1 h-full d-flex justify-content-center align-items-center">
                                <x-jet-label for="name" id="show_filter" value="{{ __('FILTER') }}"
                                    class="hover" />
                            </div>
                            ...
                            <div class="col-md-2">
                                <div class="form-floating" id="divfilterEmpOffice">
                                    <select name="filterEmpOffice" id="filterEmpOffice"
                                        class="form-control border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md mt-1 block w-full">
                            ...
                            <div class="col-md-2">
                                <div class="form-floating" id="divfilterEmpDepartment">
                                    <select name="filterEmpDepartment" id="filterEmpDepartment"
                                        class="form-control border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md mt-1 block w-full">
                            ...
                            <div class="col-md-2">
                                <div class="form-floating" id="divfilterEmpStatus">
                                    <select name="filterEmpStatus" id="filterEmpStatus"
                                        class="form-control border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md mt-1 block w-full">
                    ...
                    <div id="table_data">
                        <div class="col-span-12 sm:col-span-7 sm:justify-center scrollable">
                            ...
                            <table id="dataViewEmployees"
                                class="view-employees table table-bordered table-striped sm:justify-center table-hover">
                                <thead class="thead">
                                    ...
                                    @forelse($employees as $employee)
                                        <tr id="{{ $employee->id }}">
                                            @if (url('/') == 'http://localhost')
                                                <td>xxx, xxx x.</td>
                                            @else
                                                <td>{{ join(' ', [$employee->last_name . ' ' . $employee->suffix . ',', $employee->first_name, $employee->middle_name]) }}
                                                </td>
                                            @endif
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('dblclick', '.view-employees tr', async function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var dh = (getemployee.date_hired != null && getemployee
                        .date_hired != '1970-01-01') ? getemployee.date_hired.split(
                        '-') : '';

                    var valDateHired = (getemployee.date_hired != null && getemployee
                        .date_hired != '1970-01-01') ? getemployee.date_hired : '';
                    var sched = getemployee.weekly_schedule.split('|');

                    var valDateReg = (getemployee.date_regularized != null &&
                            getemployee.date_regularized != '01/01/1970') ? getemployee
                        .date_regularized : '';
                    ...
                    if (getemployee.profile_photo_path != null) {
                        imgProfilePhotoLocation = document.location.origin +
                            '/storage/' + getemployee.profile_photo_path;
                    } else {
                        switch (getemployee.gender) {
                            case 'M':
                                imgProfilePhotoLocation = document.location.origin +
                                    '/storage/profile-photos/default-formal-male.png';
                                break;
                            case 'F':
                                imgProfilePhotoLocation = document.location.origin +
                                    '/storage/profile-photos/default-female.png';
                                break;
                            default:
                                imgProfilePhotoLocation = document.location.origin +
                                    '/storage/profile-photos/default-photo.png';
                        }
                    }
                    ...
                    (getemployee.is_head == 1) ? $('#isHead').prop('checked', true): $(
                    ...
                    var hAdd = (getemployee.home_address != null) ? getemployee
                        .home_address + ',' : '';
                    ...
                    var pAdd = (getemployee.province != null) ? getemployee.province +
                        ',' : '';
                    var cAdd = (getemployee.country_name != null) ? getemployee
                        .country_name : '';
                    var zipCode = (getemployee.zip_code != null) ? getemployee
                        .zip_code : '';
                    var contact = (getemployee.contact_number != null) ? getemployee
                        .contact_number : '';
                    var mobile = (getemployee.mobile_number != null) ? getemployee
                        .mobile_number : '';
                    var civil_status = (getemployee.civil_status != null) ? getemployee
                        .civil_status.toUpperCase() : '';
                    var nationality = (getemployee.nationality != null) ? getemployee
                        .nationality.toUpperCase() : '';
                    var birthPlace = (getemployee.birth_place != null) ? getemployee
                        .birth_place.toUpperCase() : '';
                    var birthday = (getemployee.birthdate != null) ? getemployee
                        .birthdate : '';
                    ...
                    $('#vacationLeaves').html("Vacation Leave : " + (getLeaves.VL ?
                        getLeaves.VL : 0));
                    $('#sickLeaves').html("Sick Leave : " + (getLeaves.SL ? getLeaves
                        .SL : 0));
                    if (getemployee.gender == 'M') {
                        $('#matpatLeaves').html("Paternity Leave : " + (getLeaves.PL ?
                            getLeaves.PL : 0));
                    } else if (getemployee.gender == 'F') {
                        $('#matpatLeaves').html("Maternity Leave : " + (getLeaves.ML ?
                            getLeaves.ML : 0));
                    } else {
                        $('#matpatLeaves').html("Parental Leave : " + (getLeaves.ML ?
                            getLeaves.ML : 0));
                    }
                    $('#emergencyLeaves').html("Emergency Leave : " + (getLeaves.EL ?
                        getLeaves.EL : 0));
        ...
        $(document).on('change click', '#isHead', async function() {
            ($(this).is(":checked")) ? $("#updateRoleType").val('ADMIN'): $("#updateRoleType").val(
                'EMPLOYEE');
        });

        $(document).on('change', '#updateRoleType', async function() {
            ($(this).val() == 'ADMIN' || $(this).val() == 'SUPER ADMIN') ? $('#isHead').prop(
                "checked", true): $('#isHead').prop("checked", false);
        });
