$(document).ready(function () {
    ...
    // ==============================
    // Reason - input event with
    // character counter (replaces keyup)
    // ==============================
    $(document).on('input', '#reason', function () {
        // Character counter
        const current = $(this).val().length;
        $('#reasonCounter').text(current);
        $('#reasonCounter').css('color', current >= 240 ? 'red' : '');
    ...
    $(document).ready(function () {
        ...
        if (reason) {
            $("#reason").val(reason);
            // Restore counter on session restore
            $('#reasonCounter').text(reason.length);
            $('#reasonCounter').css('color', reason.length >= 240 ? 'red' : '');
        }
