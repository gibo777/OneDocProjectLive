...
$(document).ready(function () {
    ...
    // ===========================
    // Global Emoji Stripping
    // ===========================
    function stripEmojis(value) {
        const emojiRegex = /[\u{1F000}-\u{1FFFF}|\u{2600}-\u{27BF}|\u{FE00}-\u{FEFF}|\u{1F900}-\u{1F9FF}|\u{1FA00}-\u{1FA9F}]/gu;
        return value.replace(emojiRegex, '');
    }

    $(document).on('input', 'input[type="text"], textarea', function () {
        const cleaned = stripEmojis($(this).val());
        if (cleaned !== $(this).val()) {
            $(this).val(cleaned);
        }
    });
